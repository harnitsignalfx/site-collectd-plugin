...
def read_callback():
	try:
		...
		if response.status_code == 200:
			m = pattern.findall(response.content.decode('utf-8'))
...
def dispatch_values(type_instance,values):
	...
	#dims['dim1']='value1'
	#dims['dim2']='value2'
	#val.plugin_instance = 'site-plugin-instance'
	#val.plugin_instance += '[{dims}]'.format(dims=','.join(['='.join(d) for d in dims.items()]))
